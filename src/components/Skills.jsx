  ...
  _renderSection(name) {
    return (
      <Grid item xs={12}>
        ...
        <Typography variant="inherit" style={{ fontSize: 20, fontWeight: 'bold', textAlign: 'left' }}>
          {name}
        </Typography>
        <Divider />
      </Grid>
  ...
  _renderSkill(name, link, logo) {
    ...
    return (
      <Grid item xs={width === 'xs' || width === 'sm' ? 6 : 3} container>
        <Grid item xs>
          <a href={link} rel="noopener noreferrer" target="_blank" className="skillLink">
            <h4 className={classes.skillLogo}>
              {name}
            </h4>
          </a>
          <div style={{ textAlign: 'center', justifyContent: 'center' }}><Avatar className="skillLogo" src={logo} style={{ alignSelf: 'center', justifyContent: 'center' }}/></div>
        </Grid>
      </Grid>
    )
  }
